    ...
    def execute_job(
        ...
        with open(self.lock_file_path, "w", encoding="utf-8") as lock_file:
            ...
            if not self.no_rollout:
                if self.policy == "Sarnn":
                    ckpt_list = [f"epoch{s:04}" for s in range(800, 8000, 800)]
                elif self.policy == "Act":
                    ckpt_list = [f"epoch{s}" for s in range(100, 1000, 100)]
                elif self.policy == "DiffusionPolicy":
                    ckpt_list = [f"epoch{s:04}" for s in range(200, 2000, 200)]
                ckpt_list.append("last")
...
def plot_result(result_dirs):
    score = []
    env = None
    policy = None
    for i in range(10):
        tmp = []
        success_files = sorted(glob.glob(f"{result_dirs[i]}/**/*.txt", recursive=True))
        for file in success_files:
            with open(file, "r") as f:
                data = f.readline()
                s_list = [int(s) for s in data.split()]
                tmp.append(sum(s_list)/6)
        tmp = tmp[1:] + tmp[:1]
        score.append(tmp)
        if i == 9:
            env = success_files[0].split("/")[-2]
            policy = success_files[0].split("/")[-3]
    score_arr = np.array(score)

    mean = score_arr.mean(axis=0)
    std = score_arr.std(axis=0)
    
    if policy == "Sarnn":
        ep = 8
    elif policy == "Act":
        ep = 1
    elif policy == "DiffusionPolicy":
        ep = 2
    x = np.arange(ep*100, ep*1100, ep*100)
    plt.fill_between(x, mean + std, mean - std, alpha=0.2, color='red')
    plt.plot(x, mean, color='red')
    plt.title(f"{env} {policy}")
    plt.ylim(0, 1)
    os.makedirs("./misc/result/fig", exist_ok=True)
    plt.savefig(f"./misc/result/fig/{policy}_{env}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
...
def main():
    ...
    if not args.daily_schedule_time:
        handle_all_jobs()
        files = sorted(glob.glob(f"{os.path.dirname(os.path.abspath(__file__))}/result/*_*"), reverse=True)[:10]
        plot_result(files)
        return
